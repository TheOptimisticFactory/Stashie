...
Other top: 			BaseName^Anomalous|BaseName^Divergent|BaseName^Phantasmal|BaseName^Delirium|path^CurrencyAfflictionShard|path^CurrencyAfflictionFragment|BaseName^Enriched|BaseName^Empower|BaseName^Enlighten|BaseName^Enhance 		:1P_PriorityLoot
...
Six link:           numberoflinks >= 6                                              :9P_Dump
        ...
        public IEnumerator StashTabNamesUpdater_Thread()
        {
            while (true)
            {
                ...
                for (var index = 0; index < realNames.Count; ++index)
                {
                    ...
                    if (cachedName != null && cachedName.Equals(realNames[index])) continue;
